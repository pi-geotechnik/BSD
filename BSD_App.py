...
with st.expander("ℹ️ About this app"):
    ...
    **Version 2.1, June 2025**
    ...
    As worked out in the above references, designing rock fall protection measures with only one *design block* may result in unreliable trajectories (i.e. kinetic energies, bounce heights and runout).  
    Block size distributions (BSDs) derived from debris fields can be very subjective.  
    ...
    **This application visualizes block size distributions, fits distribution functions to them and provides block lists of the fitted distribution for rockfall simulations (e.g. with [THROW](https://pigeotechnik.pythonanywhere.com/)).**  
    It offers the determination of more meaningful BSDs, which is also possible with a limited number of block measurements (min. 65 blocks are recommended).  
    ...
    **Expert opinion is required!**  
    Based on a defined worst-case scenario (with a certain annuality), events/block sizes with higher return periods may be neglected/accepted (cut off).  
